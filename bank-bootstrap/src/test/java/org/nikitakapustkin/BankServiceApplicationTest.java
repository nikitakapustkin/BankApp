...
class BankServiceApplicationTest {
  ...
  void happy_path_create_users_accounts_and_do_operations() throws Exception {
    ...
    mvc.perform(
            authorized(
                post("/accounts/{id}/deposit", aliceAcc)
    ...
    mvc.perform(
            authorized(
                post("/accounts/{from}/transfer", aliceAcc)
                    .contentType(MediaType.APPLICATION_JSON)
    ...
    mvc.perform(
            authorized(
                post("/accounts/{id}/withdraw", bobAcc)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"amount\":10.00}")))
        .andExpect(status().isOk());

    BigDecimal aliceBalance =
        readBigDecimal(
    ...
    String aliceDetailsJson =
        mvc.perform(authorized(get("/accounts/{id}", aliceAcc)))
            .andExpect(status().isOk())
    ...
    assertThat(aliceDetails.get("transactions"))
        .anySatisfy(node -> assertThat(node.get("type").asText()).isIn("DEPOSIT", "TRANSFER"));
  ...
  void withdraw_more_than_balance_returns_409_and_domain_error_code() throws Exception {
    ...
    mvc.perform(
            authorized(
                post("/accounts/{id}/withdraw", aliceAcc)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("{\"amount\":10.00}")))
  ...
  void getUsers_invalidSex_returns_400_invalid_argument() throws Exception {
    mvc.perform(authorized(get("/users").param("sex", "WRONG")))
  ...
  void deleteUser_with_accounts_and_friendships_cascades_and_returns_204() throws Exception {
    ...
    mvc.perform(authorized(delete("/users/{userId}", aliceId))).andExpect(status().isNoContent());

    mvc.perform(authorized(get("/users/{userId}", aliceId))).andExpect(status().isNotFound());
  ...
  private UUID createUser(String login, String name, int age, String sex, String hairColor) {
    ...
    ImportUserCommand command =
        new ImportUserCommand(
            userId, login, name, age, Sex.valueOf(sex), HairColor.valueOf(hairColor));
