...
public final class EventPayloadRegistry {
  private static final PayloadMapping UNKNOWN_USER_MAPPING =
      mapping(
          UnknownEventPayload.class,
          payload -> new EventFields(payload.entityId(), payload.description()));
  private static final PayloadMapping UNKNOWN_ACCOUNT_MAPPING =
      mapping(
          UnknownEventPayload.class,
          payload -> new EventFields(payload.entityId(), payload.description()));
  private static final Map<String, PayloadMapping> USER_EVENT_MAPPINGS =
      Map.of(
          ...
          EventTypes.FRIEND_ADDED,
              mapping(
                  ...
                  payload -> new EventFields(payload.userId(), payload.description())),
          EventTypes.FRIEND_REMOVED,
              mapping(
                  FriendRemovedPayload.class,
                  payload -> new EventFields(payload.userId(), payload.description())));
  private static final Map<String, PayloadMapping> ACCOUNT_EVENT_MAPPINGS =
      Map.of(
          ...
          EventTypes.ACCOUNT_DEPOSIT,
              mapping(
                  ...
                  payload -> new EventFields(payload.accountId(), payload.description())),
          EventTypes.ACCOUNT_WITHDRAWAL,
              mapping(
                  AccountWithdrawnPayload.class,
                  payload -> new EventFields(payload.accountId(), payload.description())),
          EventTypes.ACCOUNT_TRANSFER,
              mapping(
  ...
  public static PayloadInfo resolveUserPayload(
      String eventType, JsonNode payload, ObjectMapper objectMapper) {
    return resolvePayload(
        eventType, payload, objectMapper, USER_EVENT_MAPPINGS, UNKNOWN_USER_MAPPING);
  }

  public static PayloadInfo resolveAccountPayload(
      String eventType, JsonNode payload, ObjectMapper objectMapper) {
    return resolvePayload(
        eventType, payload, objectMapper, ACCOUNT_EVENT_MAPPINGS, UNKNOWN_ACCOUNT_MAPPING);
  }

  private static PayloadInfo resolvePayload(
      String eventType,
      JsonNode payload,
  ...
  private static <T> PayloadMapping mapping(
      Class<T> payloadClass, Function<T, EventFields> extractor) {
