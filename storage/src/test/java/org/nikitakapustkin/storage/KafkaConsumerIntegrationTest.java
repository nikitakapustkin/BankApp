...
@EmbeddedKafka(partitions = 1, topics = {"client-topic", "account-topic", "transaction-topic"})
@DirtiesContext
class KafkaConsumerIntegrationTest {
    ...
    void consumes_user_event_from_kafka_and_persists() throws Exception {
        ...
        kafkaTemplate.send("client-topic", userId.toString(), objectMapper.writeValueAsString(envelope))
        ...
        UserCreatedPayload actualPayload = objectMapper.readValue(saved.getPayload(), UserCreatedPayload.class);
    ...
    void consumes_account_event_from_kafka_and_persists() throws Exception {
        ...
        EventEnvelope<AccountDepositedPayload> envelope = new EventEnvelope<>(
                eventId,
        ...
        kafkaTemplate.send("account-topic", accountId.toString(), objectMapper.writeValueAsString(envelope))
                .get(5, TimeUnit.SECONDS);
        ...
        AccountDepositedPayload actualPayload = objectMapper.readValue(saved.getPayload(), AccountDepositedPayload.class);
    ...
    void consumes_transaction_event_from_kafka_and_persists() throws Exception {
        ...
        kafkaTemplate.send("transaction-topic", accountId.toString(), objectMapper.writeValueAsString(envelope))
                .get(5, TimeUnit.SECONDS);
        ...
        TransactionCreatedPayload actualPayload = objectMapper.readValue(
                saved.getPayload(),
    ...
    private UserEvent awaitUserEvent(UUID eventId) throws InterruptedException {
        ...
        while (System.currentTimeMillis() < deadline && found == null) {
            found = userEventRepository.findAll().stream()
    ...
    private AccountEvent awaitAccountEvent(UUID eventId) throws InterruptedException {
        ...
        while (System.currentTimeMillis() < deadline && found == null) {
            found = accountEventRepository.findAll().stream()
                    .filter(e -> eventId.equals(e.getEventId()))
                    .findFirst()
                    .orElse(null);
            if (found == null) {
    ...
    private TransactionEvent awaitTransactionEvent(UUID transactionId) throws InterruptedException {
        ...
        while (System.currentTimeMillis() < deadline && found == null) {
            found = transactionEventRepository.findAll().stream()
