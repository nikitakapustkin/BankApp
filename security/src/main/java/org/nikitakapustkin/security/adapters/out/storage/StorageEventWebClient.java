...
public class StorageEventWebClient implements StorageEventClientPort {
    ...
    public List<StorageEventResponseDto> getEvents(
        ...
        Mono<List<StorageEventResponseDto>> response = storageClient.get()
                .uri(uriBuilder -> uriBuilder
                        ...
                        .queryParamIfPresent("transactionType", Optional.ofNullable(transactionType))
    ...
    private Mono<? extends Throwable> toStorageException(org.springframework.web.reactive.function.client.ClientResponse response) {
        int status = response.statusCode().value();
        return response.bodyToMono(String.class)
                .defaultIfEmpty("")
                .map(body -> new BankServiceException(
                        status,
                        mapErrorCode(status),
