...
public class BankWebClientSupport {
    ...
    public Mono<? extends Throwable> toBankException(ClientResponse response) {
        ...
        return response.bodyToMono(ApiError.class)
                ...
                .map(error -> new BankServiceException(
                        error.status() > 0 ? error.status() : status,
                        resolveErrorCode(error.error(), status),
                        resolveMessage(error.message()),
                        error.path()
                ))
                .onErrorResume(ex -> Mono.just(new BankServiceException(
                        status,
                        resolveErrorCode(null, status),
                        "Error from main service",
                        null
                )));
    }
    ...
    private static ErrorCode resolveErrorCode(org.nikitakapustkin.bank.contracts.errors.ErrorCode raw, int status) {
        if (raw != null) {
            try {
