...
class BankOpenApiContractTest {
  ...
  void bank_openapi_contains_endpoints_used_by_security() {
    Path specPath =
        Path.of("..", "bank-bootstrap", "src", "main", "resources", "openapi", "bank-openapi.yaml")
            .normalize()
            .toAbsolutePath();

    assertThat(Files.exists(specPath)).as("bank-openapi.yaml must exist at %s", specPath).isTrue();
