...
public class WithdrawMoneyService implements WithdrawMoneyUseCase {
    ...
    public void withdraw(WithdrawMoneyCommand cmd) {
        ...
        var account = loadAccountPort.loadAccount(cmd.getAccountId())
                .orElseThrow(() -> new AccountNotFoundException("Account not found: " + cmd.getAccountId()));

        account.withdraw(cmd.getAmount());
        updateAccountStatePort.updateAccount(account);

        Transaction saved = recordTransactionPort.recordTransaction(Transaction.builder()
        ...
        String description = "Withdraw " + cmd.getAmount().toPlainString() + " from account " + account.getId();
        publishAccountEventPort.publishAccountEvent(DomainEvent.now(
                account.getId(),
                EventType.ACCOUNT_WITHDRAWAL,
                description,
                correlationId,
                new AccountWithdrawnEventData(
                        account.getId(),
                        cmd.getAmount(),
                        description
                )
